...
class event_model extends CI_Model
{
    function newsListing($filter, $page, $segment)
    {
        ...
        if(!empty($filter['location'])) {
            $this->db->where('news.location', $filter['location']);
        }
        $this->db->from('tbl_event_register as news'); 
    ...
    {
        $this->db->trans_start();
        $this->db->insert('tbl_event_register', $newsInfo);
    ...
    function updateNews($newsInfo, $row_id)
    {
        ...
        $this->db->update('tbl_event_register', $newsInfo);
        return TRUE;
    }
    ...
    {
        $this->db->where('row_id', $row_id);
        $this->db->update('tbl_event_register', $newsAndEventInfo);
        return TRUE;
    }
}