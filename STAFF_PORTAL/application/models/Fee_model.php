...
class Fee_model extends CI_Model
{
    ...
    public function getCourseFeeLimitForConcession($student_row_id, $con_year)
    {
        $this->db->select('student.term_name, student.stream_name, student.gender, year_info.class as year_term_name, year_info.stream as year_stream_name');
        $this->db->from('tbl_students_info as student');
        $this->db->join('tbl_student_class_year_wise as year_info', 'year_info.stud_row_id = student.row_id AND year_info.intake_year = '.(int)$con_year.' AND year_info.is_deleted = 0', 'left');
        $this->db->where('student.row_id', $student_row_id);
        $this->db->where('student.is_deleted', 0);
        $studentInfo = $this->db->get()->row();
        if (empty($studentInfo)) {
            return 0;
        }
        $term_name = !empty($studentInfo->year_term_name) ? $studentInfo->year_term_name : $studentInfo->term_name;
        $stream_name = !empty($studentInfo->year_stream_name) ? $studentInfo->year_stream_name : $studentInfo->stream_name;
        $gender = $studentInfo->gender;
        $this->db->select('SUM(fee.fee_amount_state_board) as total_fee');
        $this->db->from('tbl_admission_fee_structure as fee');
        $this->db->where('fee.fees_type', 7);
        $this->db->where('fee.fee_year', $con_year);
        $this->db->where('fee.is_deleted', 0);
        $this->db->where_in('fee.term_name', array($term_name, 'ALL'));
        $this->db->where_in('fee.stream_name', array($stream_name, 'ALL'));
        $this->db->where_in('fee.gender', array($gender, 'ALL'));
        $courseFee = $this->db->get()->row();
        return !empty($courseFee->total_fee) ? (float)$courseFee->total_fee : 0;
    }

    public function getLatestPendingBalanceForConcession($student_row_id, $con_year)
    {
        $this->db->select('fee.pending_balance');
        $this->db->from('tbl_new_admission_overall_fee_payment_info as fee');
        $this->db->where('fee.application_no', $student_row_id);
        $this->db->where('fee.payment_year', $con_year);
        $this->db->where('fee.is_deleted', 0);
        $this->db->order_by('fee.row_id', 'DESC');
        $this->db->limit(1);
        $pendingInfo = $this->db->get()->row();
        if (empty($pendingInfo)) {
            return null;
        }
        return isset($pendingInfo->pending_balance) ? (float)$pendingInfo->pending_balance : 0;
    }

    public function getStudentConcessionTotal($student_row_id, $con_year, $exclude_row_id = null)
    {
        $this->db->select('SUM(fee.fee_amt) as total_concession');
        $this->db->from('tbl_student_fee_concession as fee');
        $this->db->where('fee.application_no', $student_row_id);
        $this->db->where('fee.year', $con_year);
        $this->db->where('fee.is_deleted', 0);
        if (!empty($exclude_row_id)) {
            $this->db->where('fee.row_id !=', $exclude_row_id);
        }
        $concession = $this->db->get()->row();
        return !empty($concession->total_concession) ? (float)$concession->total_concession : 0;
    }
   ...
   public function getFeeInstallmentById($row_id) {
    ...
    public function getManagementFeeForReport($filter='')
    {
        ...
        if(!empty($filter['term_name']) && $filter['term_name'] != 'ALL'){
            $this->db->where('fee.term_name', $filter['term_name']);
        }
        $this->db->order_by('fee.row_id', 'ASC');
        $this->db->order_by('fee.receipt_number', 'ASC');
    ...
    public function getManagementFeeForDateWiseReport($filter='')
    {
        $this->db->select('fee.*,std.stream_name');
        $this->db->from('tbl_new_admission_overall_fee_payment_info as fee');      
        ...
        if(!empty($filter['stream_name']) && $filter['stream_name'] != 'ALL'){
            $this->db->where('std.stream_name', $filter['stream_name']);
                ...
                log_message('debug','filter--aswdefrgt----->'.print_r($filter,true));
    ...
    public function getBankSettlementFeeForReport($filter='')
    {
        $this->db->from('tbl_new_admission_overall_fee_payment_info as fee');      
        if(!empty($filter['date_from'])){
    ...
    public function updatefeeSettleStatusNew($feeInfo, $order_id) {
    ...
    public function getLastReceiptNoNew($year){
        $this->db->select('fee.receipt_number');
        $this->db->from('tbl_new_admission_overall_fee_payment_info as fee');
        $this->db->where('fee.is_deleted', 0);
        $this->db->where('fee.payment_year', $year);
    ...
    public function getLastReceiptNo($year){
        ...
        $this->db->where('fee.is_deleted', 0);
        $this->db->where('fee.payment_year', $year);
        $this->db->like('fee.receipt_number', substr((string)$year, -2) . 'T', 'after');
        $this->db->order_by('fee.row_id', 'DESC');
        $this->db->limit(1);
        $query = $this->db->get();
        $lastReceiptInfo = $query->row();
        return $this->formatReceiptNumber($year, 'T', !empty($lastReceiptInfo) ? $lastReceiptInfo->receipt_number : '');
    }
    ...
    function checkStudentIdExistsForConcession($application_no,$year){
        $this->db->from('tbl_student_fee_concession as fee');
        $this->db->where('fee.application_no', $application_no);
        $this->db->where('fee.year',$year);
        $this->db->where('fee.is_deleted', 0);
    ...
    public function getFeeStructureName($feeType = ''){
        $this->db->from('tbl_fees_name as fee'); 
        $feeType = strtoupper(trim((string)$feeType));
        if($feeType == 'MANAGEMENT FEE'){
            $this->db->where_not_in('fee.row_id', array(1));
        }else if($feeType == 'CONTRIBUTION FEE'){
            $this->db->where_in('fee.row_id', array(1));
        }
        $this->db->order_by('fee.priority','asc');       
        $query = $this->db->get();
        $result = $query->result();
        return $result;
    }

    public function getFeeReceiptPrintInfoReport($receipt_number,$application_no,$fees_type){
        $this->db->select('stru.fees_type,fee.payment_date,type.fee_name, fee.receipt_no,
        fee.receipt_number,payment.application_no,fee.paid_amount');
        $this->db->from('tbl_fees_paid_by_receipt_v2 as fee'); 
        $this->db->join('tbl_admission_fee_structure as stru', 'stru.row_id = fee.fee_structure_id','left');
        $this->db->join('tbl_fees_name as type', 'type.row_id = fee.fee_type_id','left');
        $this->db->join('tbl_new_admission_overall_fee_payment_info as payment', 'payment.row_id = fee.receipt_number');
        $this->db->where('fee.receipt_number', $receipt_number);
        $this->db->where('payment.application_no', $application_no);
        $this->db->where('type.row_id', $fees_type);
        $query = $this->db->get();
        $result = $query->result();
        return $result;
    }

    public function getFeePaidInfoForReportForReceipt($application_no,$term_name,$fee_year){
            $this->db->select('
            fee.payment_date,
            fee.row_id,
            fee.receipt_number,
            fee.application_no,
            std.student_name as name,
        ...
        $this->db->where('fee.application_no', $application_no);
        $this->db->where('fee.payment_count', 1);
        $this->db->where('fee.payment_year', $fee_year);
        $this->db->where('fee.is_deleted', 0);
        $this->db->order_by('fee.row_id');
        $query = $this->db->get();
        return $query->row();
    }

    public function getSpecialFeeInfoByYear($term, $year, $filter = []) {
        $this->db->from('tbl_special_fees as spcl');
        $this->db->where('spcl.term_name', $term);
        $this->db->where('spcl.year', $year);
        $this->db->where_in('spcl.fee_type', [$filter['student_fee_type'],'ALL']);
        $this->db->where('spcl.is_deleted', 0);
        $this->db->order_by('spcl.priority', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }

    public function getSpecialFeeReceiptsByFilter($filter)
    {
        $this->db->select('
            fee.receipt_number,
            fee.application_no,
        ...
        if (!empty($filter['fee_year'])) {
            $this->db->where('fee.payment_year', $filter['fee_year']);
        }
        if (!empty($filter['date_from'])) {
            $this->db->where('fee.payment_date >=', date('Y-m-d', strtotime($filter['date_from'])));
        }
        ...
        if (!empty($filter['payment_type']) && $filter['payment_type'] != 'ALL') {
            $this->db->where('fee.payment_type', $filter['payment_type']);
        }
        $this->db->group_by('fee.application_no');
        $this->db->order_by('fee.payment_date', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function checkOrderIdExistsInFeesPaymentLog($order_id){
        $this->db->from('tbl_admission_payment_log_paytm as fee');
        $this->db->where('fee.order_id', $order_id);
        $this->db->where('fee.is_deleted', 0);
        $query = $this->db->get();
        return $query->row();
